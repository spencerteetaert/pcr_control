...
class PCR_Learned_Model(nn.Module):
    def __init__(self, prediction_horizon, linear_layers = [[10, 10], [100]], lstm = 30, configuration_state = 0, device=default_device):
        '''
        Arguments 
        ...
        self.configuration_states = None
        if configuration_state != 0:
            self.configuration_states = nn.Parameter(torch.Tensor(5, configuration_state), requires_grad=True)
