...
class PCRDataSet(Dataset):
    def __init__(self, folder, prediction_horizon, feedback_horizon=0, min_travel_distance=0, run_types=['SUCCESS'], device=default_device, debug=False):
        '''
        Args: 
        ...
        for file_meta_data in files_meta_data:
            ...
            for i in range(1,len(lines)):
                ...
                for j in range(0, len(lines_motor)):
                    ...
                    # Break when / if we run out of motor readings 
                    ...
                    if time > start_time:
                        ...
                        if aurora_curr_idx == len(lines_aurora):
                            ...
                            raise NotImplementedError("Unknown behaviour. Must fix this before use.")
                        else:
                            start_time = float(lines_aurora[aurora_curr_idx].decode().split(",")[0])
                ...
                assert(len(to_add_mappings) == aurora_end_idx - aurora_start_idx), "Data misaligned. Something went wrong."

                self.aurora_data_map += to_add_mappings
                self.motor_data += to_add_data[to_add_start_idx:to_add_end_idx]

                # Add Aurora data 
                for j in range(aurora_start_idx, aurora_end_idx): # -1 here fixes, not ideal 
        ...
        # Sanity check 
        assert(len(self.aurora_data) == len(self.aurora_data_map)), "Data misaligned. Something went wrong."
        assert(len(self.indexable_aurora_data) <= len(self.aurora_data)), "Data misaligned. Something went wrong."
    ...
    def __getitem__(self, _idx):
        ...
        Label: 
        - M motor commands from last known location 
        '''

        idx = self.indexable_aurora_data[_idx]

        assert(self.aurora_data[idx]['trial_prefix'] == self.aurora_data[idx-1]['trial_prefix']), "Data misaligned. Something went wrong."
        ...
        position_data = torch.tensor([
            ...
            self.aurora_data[idx]['configuration_number']
            ]).to(self.device)
